...
FILE *in,*out; char r[50],b[1000][50]; int tot,len1,len2,fst[8400000],srt[8400000],to[1000],nxt[500000];
...
void ready()
{
	int num; register int i=0;
	in=fopen("dictionary.txt","r");
	while (fscanf(in,"%s",a[++i].s)!=EOF)
	{
		num=hash(a[i].s);
		nxt[i]=fst[num];
		fst[num]=i;
