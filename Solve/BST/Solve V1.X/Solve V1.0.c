...
FILE *in,*out; stu a[500000],b[1000]; typedef struct {int num; char s[50];} dic;
ptr root1,root2; int n,len1,len2; char r[50]; dic in1[500000],in2[500000],cpy1[500000],cpy2[500000];
...
int exist(ptr now) {
	if (now==NULL) return 0; int t=strcmp(r,now->s);
	if (t>0) return exist(now->rs);
	else if (t<0) return exist(now->ls); return 1;
}
...
void init() {
	...
	while (fscanf(in,"%s",a[len1++].s)!=EOF); fclose(in); root1=build(a,0,len1);
	in=fopen("stopwords.txt","r");
	while (fscanf(in,"%s",b[len2++].s)!=EOF); fclose(in); root2=build(b,0,len2);
	in=fopen("article1.txt","r");
	while (read()) if (!exist(root2)) go1(root1); fclose(in);
	in=fopen("article2.txt","r");
	while (read()) if (!exist(root2)) go2(root1); fclose(in);
	len1=len2=0; scanf("%d",&n); return;
}
...
void solve() {
	...
	dfs(root1),n=min(min(n,len1),len2);
