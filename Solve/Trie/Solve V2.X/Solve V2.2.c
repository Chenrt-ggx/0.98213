...
void init()
{
	in=fopen("dictionary.txt","rb"); ins(); fclose(in);
	in=fopen("stopwords.txt","rb"); del(); fclose(in);
	in=fopen("article1.txt","rb"); count1(); fclose(in);
	in=fopen("article2.txt","rb"); count2(); fclose(in);
	return;
}
