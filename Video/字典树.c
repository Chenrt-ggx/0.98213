...
{
	int tmp=0,son;
	for (ri i=0;newword[i];i++)
	{
		son=newword[i]-'a';
		if (!pol[tmp].s[son]) pol[tmp].s[son]=++top;
		tmp=pol[tmp].s[son];
	}
	pol[tmp].con1+=!flag;
	pol[tmp].con2+=flag;
...
void del()
{
	...
	{
		son=newword[i]-'a';
		if (!pol[tmp].s[son]) return;
		else tmp=pol[tmp].s[son];
	}
	pol[tmp].con1=0;
	pol[tmp].con2=0;
	return;
...
void add()
{
	...
	if (pol[tmp].con1)
	{
		...
		strcpy(article1[topart1++].s,newword);
	}
	if (pol[tmp].con2)
	{
		article2[topart2].num=pol[tmp].con2;
		strcpy(article2[topart2++].s,newword);
	}
	return;
}
...
int main()
{
	...
	while (read()) ins(0); fclose(in);
	in=fopen("article2.txt","r");
	while (read()) ins(1); fclose(in);
	in=fopen("stopwords.txt","r");
	while (~fscanf(in,"%s",newword)) del(); fclose(in);
	in=fopen("dictionary.txt","r");
	while (~fscanf(in,"%s",newword)) add(); fclose(in);
	scanf("%d",&n);
