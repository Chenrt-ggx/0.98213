...
FILE *in,*out; char r[50]; int len1,len2,fst[134220000],nxt[500000];
...
int hash(char *s)
{
	...
	for (register int i=0;ch=*s++;i++)
	{
		if (i&1)  ans^=(~((ans<<11)^ch^(ans>>5)));
		else  ans^=((ans<<7)^ch^(ans>>3));
	}
	return ans&134217727;
}
